...
export default class Admin extends PureComponent {
  ...
  render() {
    ...
    return (
      <div className="Admin vertical space-children">       
            ...
            <div className="presentation-container">
              <div className="presentation-side">
                ...
                <div className="presentation-overlays">
                  <div>Presentation Screen&nbsp;<button className="dd-bordered overlay-button" onClick={this.launchPresentation} disabled={launchDisabled || !this.bigScreenUrl()}>Launch Presentation</button></div>
                </div>
              </div>
              <div className="presentation-side">
                <PresentationDriver
        ...
        <div>
          <input type="text" value={backgroundUrl} onChange={this.onBackgroundUrlChange} placeholder="Custom background image URL. Suggested at least 700px high and wide." className="dd-bordered background-url" />
        </div>
      </div>
    )
  }
  ...
  updateSessionContestantName = contestantName => {
    this.sessionsRef().child(sessionId).update({contestantName})
    if (this.state.publicSessions[sessionId]) this.publicSessionRef().child(sessionId).update({contestantName})
  }

  onBackgroundUrlChange = e => this.backgroundUrlRef().set(e.target.value)

  createSession = () => this.sessionsRef().child('default').set({name: 'default'})
  ...
  bigScreenUrl = () => this.state.longLivedToken ? `?page=bigScreen&sessionId=${encodeURIComponent(sessionId)}&token=${encodeURIComponent(this.state.longLivedToken)}` : null
}
